  ...
  this.loadfile = function()
  {
    ...
    $.getJSON(obj.jsonfile, function(data) {
      ...
      $.each(data["tilesets"], function( index_tileset, value_tileset ) {
          ...
          if (value_tileset["name"]=="top")
          {
            obj.topTiles=new Anim("top_tiles",0,"data/"+value_tileset["image"],obj.tileSize);
            obj.firstTopTileIndex=value_tileset["firstgid"];
          }
      ...
      $.each(data["layers"], function( index_layer, value_layer ) {
          ...
          if ((value_layer["type"]=="tilelayer")&&(value_layer["name"]=="top"))
            obj.topMap=value_layer["data"];
        ...
        for (var x=drawX;x<drawX+drawW;x++)
        {
          if ((this.topMap[i]>0)||(height==0))
          {
            ...
            {          
              ...
              if (height==0)//floor
              {
                frameXimg=((this.mainMap[i]-this.firstMainTileIndex)%this.tiles.nbC)*this.tileSize[0];
                frameYimg=(Math.floor((this.mainMap[i]-this.firstMainTileIndex)/this.tiles.nbC))*this.tileSize[1];
              ...
              else//high parts (only if topMap[i]>0)
              {
                frameXimg=((this.topMap[i]-this.firstTopTileIndex)%this.topTiles.nbC)*this.tileSize[0];
                frameYimg=(Math.floor((this.topMap[i]-this.firstTopTileIndex)/this.topTiles.nbC))*this.tileSize[1];
                canvas.drawImage(this.topTiles.img,frameXimg,frameYimg,
